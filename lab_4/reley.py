...
def reley():
  ...
  t.add_row(['Выборочное математическое ожидание распределения Рэле', round(mk, 3)])
  t.add_row(['Выборочная дисперсия распределения Рэле с известным МО', round(find_dispersion(x_neumann, m_ne), 3)])
  t.add_row(['Выборочная дисперсия распределения Рэле с неизвестным МО', round(find_dispersion(x_neumann, mk), 3)])
